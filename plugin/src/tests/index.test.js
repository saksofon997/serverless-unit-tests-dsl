...
describe("serverless plugin slack", () => {
  ...
  describe("single function deployment", () => {
    test("it sends message", () => {
      ...
      expect(SlackServerlessPlugin.sendWebhook).toHaveBeenCalledWith({
        body:
          '{"text":"`jd` deployed function `bar` to environment `staging` in service `foobar`","username":"jd","icon_emoji":":cloud:"}',
  ...
  describe("default parameters properly", () => {
    test("it sends message", () => {
      ...
      expect(SlackServerlessPlugin.sendWebhook).toHaveBeenCalledWith({
        body:
          '{"text":"`jd` deployed service `foobar` to environment `dev`","username":"jd","icon_emoji":":cloud:"}',
        headers: { "Content-type": "application/json" },
        method: "POST",
        url: "https://example.com",
      });
